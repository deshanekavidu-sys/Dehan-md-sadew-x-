...
case 'setting': {
    ...
    // .settings  -> show current settings + how to edit
    if (!args[0]) {
        let list = `*↳ ❝ [⚙️ 𝗔𝗸𝗶𝗿𝗮 𝗚𝗶𝗿𝗹 𝗦𝗲𝘁𝘁𝗶𝗻𝗴𝘀 ⚙️] ¡! ❞*\n\n`;
        for (const key of Object.keys(editableKeys)) {
            ...
            list += `*₊❏❜ ⋮ ${meta.label} (${key}):* ${value}\n`;
        }
        list += `\n*📋 How to edit:*\n${sessionConfig.PREFIX}settings <key> <value>\n\n`;
        list += `*Examples:*\n`;
        list += `${sessionConfig.PREFIX}settings AUTO_VIEW_STATUS true\n`;
        list += `${sessionConfig.PREFIX}settings AUTO_LIKE_STATUS false\n`;
        list += `${sessionConfig.PREFIX}settings MODE private\n`;
        list += `${sessionConfig.PREFIX}settings PREFIX .\n`;
        list += `\n> *𝗔esthatic 𝗤ueen 𝗕y 𝗖hamod 𝜗𝜚⋆*`;

        return await socket.sendMessage(sender, {
            image: { url: sessionConfig.AKIRA_IMG || config.AKIRA_IMG },
            caption: list
        }, { quoted: msg });
    }
