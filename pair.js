...
const config = {
    ...
    BOT_NAME: 'Kadiya',
    // "Always Online" — when true, the bot keeps refreshing WhatsApp's
    // presence to 'available' so the number shows as online 24/7.
    // Toggle with `.settings alwaysonline on/off`.
    ALWAYS_ONLINE: 'false',
    // TMDB (The Movie Database) — free, official, legal metadata API.
    ...
    // only — this bot does NOT source or distribute pirated movie files.
    TMDB_API_KEY: process.env.TMDB_API_KEY || ''
};

// ══════════════════════════════════════════════════════════════════
// 🅱️ BOT NAME STYLING — converts a plain name the user sets via
// `.settings botname <name>` into the same 𝗠𝗮𝘁𝗵 𝗦𝗮𝗻𝘀 𝗕𝗼𝗹𝗱 unicode
...
const COMMANDS_REGISTRY = [
    ...
    { cmd: 'movie', desc: 'ᴍᴏᴠɪᴇ ɪɴꜰᴏ + ᴛʀᴀɪʟᴇʀ (ʟᴇɢᴀʟ, ᴠɪᴀ ᴛᴍᴅʙ)', category: 'Download' },
    ...
    { cmd: 'animeinfo', desc: 'ᴀɴɪᴍᴇ ɪɴꜰᴏ + ᴛʀᴀɪʟᴇʀ (ᴠɪᴀ ᴍᴀʟ/ᴊɪᴋᴀɴ)', category: 'Fun' },
...
// ══════════════════════════════════════════════════════════════════
// 🟢 ALWAYS ONLINE — WhatsApp presence ('available') fades back to
...
async function loadUserConfig(number) {
    try {
        ...
        // returning the saved config alone — this way, whenever a new
                ...
                try {
                    ...
                    await socket.sendMessage(userJid, {
                        ...
                        caption: formatMessage(
                            `*↳ ❝ [🎀 𝗪𝗲𝗹𝗹𝗰𝗼𝗺𝗲 𝗧𝗼 ${getBotName(freshConfig)} 𝗠𝗜𝗡𝗜 🎀] ¡! ❞*`,
  ...
  const replyFq = text => socket.sendMessage(sender, { text, contextInfo: arabianCtx() }, { quoted: fq });
	...
	// ════════════ MENU ════════════
      ...
      await socket.sendMessage(sender, {
        image: { url: akira },
        caption: `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗠𝗲𝗻𝘂 🎀] ¡! ❞*
...
> *𝗔esthatic 𝗤ueen 𝗕y 𝗜ꜱᴀɴᴋᴀ 𝜗𝜚⋆*`,
    ...
    case 'ping': {
      ...
      await socket.sendMessage(sender, {
        image: { url: akira },
        caption: `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗣𝗶𝗻𝗴 🎀] ¡! ❞*\n\n` +
			     `┏━━━━━°⌜ \`赤い糸\` ⌟°━━━━━┓\n` +
                 `┃₊❏❜ ⋮🏓 𝙿𝙾𝙽𝙶 : _pong!_\n` +
                 `┃₊❏❜ ⋮⚡ 𝚂𝙿𝙴𝙴𝙳 : ${ms}ms\n` +
                 `┃₊❏❜ ⋮⏱️ 𝚄𝙿𝚃𝙸𝙼𝙴 : ${getUptime()}\n` +
...
case 'alive': {
    ...
    const title = `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗔𝗹𝗶𝘃𝗲 🎀] ¡! ❞*`;
...
// ════════════ SYSTEM ════════════
    ...
    case 'system': {
      ...
      const sysInfo = `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗦𝘆𝘀𝘁𝗲𝗺 🎀] ¡! ❞*\n\n` +
		              `┏━━━━━°⌜ \`赤い糸\` ⌟°━━━━━┓\n` +
                      `┃ *⏱️ 𝚄𝙿𝚃𝙸𝙼𝙴:* ${uptime}\n` +
...
case 'ytmp3': {
    try {
        ...
        const caption = `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗩𝗶𝗱𝗲𝗼 🎀] ¡! ❞*\n\n` +
                        `> *\`🎵 𝚃𝙸𝚃𝙻𝙴 :\`* ${video.title}\n` +
...
case 'playvid': {
    try {
        ...
        let caption = `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗩𝗶𝗱𝗲𝗼 🎀] ¡! ❞*\n\n` +
                        `🎬 *TITLE :* ${video.title}\n` +
...
case 'facebook': {
    try {
        ...
        const caption = `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗙𝗮𝗰𝗲𝗯𝗼𝗼𝗸 🎀] ¡! ❞*\n\n` +
                        `🎬 *TITLE :* ${title}\n` +
                        `⏱️ *DURATION :* ${duration}\n` +
                        `📺 *QUALITY :* ${quality}\n` +
...
case 'tt': {
    try {
        ...
        const caption = `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗧𝗶𝗸𝗧𝗼𝗸 🎀] ¡! ❞*\n\n` +
...
case 'pinterest': {
    try {
        ...
        const caption = `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗣𝗶𝗻𝘁𝗲𝗿𝗲𝘀𝘁 🎀] ¡! ❞*\n\n` +
...
case 'akira': {
    ...
    const title = `🎀 *${getBotName(sessionConfig)} 𝗔𝗶 𝗚𝗶𝗿𝗹𝗳𝗿𝗲𝗻𝗱* 🎀`;
...
// ════════════ ACTIVE ════════════
    ...
    case 'active': {
      ...
      const responseText = `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗦𝗲𝘀𝘀𝗶𝗼𝗻𝘀 🎀] ¡! ❞*\n\n` +
                           `> *\`📡 𝙲𝙾𝚄𝙽𝚃 :\`* ${nums.length}\n\n` +
...
// ════════════ NPM ════════════
    ...
    case 'npm': {
      ...
      try {
        ...
        const npmInfo = `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗡𝗣𝗠 🎀] ¡! ❞*\n` +
                        `⊹₊⟡⋆ 𝗡𝗮𝗺𝗲 - ${d.name} 𝜗𝜚⋆\n\n` +
...
case 'setting': {
    ...
    if (!sub) {
        ...
        const panel =
`*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗦𝗲𝘁𝘁𝗶𝗻𝗴𝘀 🎀] ¡! ❞*
...
│🟢 𝗔𝗟𝗪𝗔𝗬𝗦 𝗢𝗡𝗟𝗜𝗡𝗘 ⋮ ${on(sessionConfig.ALWAYS_ONLINE) ? 'ON ✅' : 'OFF ❌'}
╰──────────────────<𝟑 .ᐟ
...
│ ${p}settings alwaysonline on/off
...
│ ${p}movie <name>  ⟶ movie info + trailer
    ...
    try {
        switch (sub) {
            case 'botname': {
                ...
                if (newName.length > 20) return reply(`⚠️ Keep the bot name under 20 characters.`);
            ...
            case 'onlinealways': {
                ...
                await reply(`✅ Always Online turned *${val.toUpperCase()}*.\n_When ON, the bot keeps refreshing WhatsApp presence so your number always shows "online". This uses more battery/data on the host device and makes the number visibly active 24/7 — some people prefer it off for privacy._`);
                break;
            }
            case 'ownername': {
                const newName = args.slice(1).join(' ').trim();
...
case 'channelsong': {
    ...
    try {
        ...
        // return HTTP 200 with an HTML/JSON error page — without this check
        ...
        if (!audioBuffer) {
            ...
            return reply(`❌ MP3 file එක ගන්න බැරි උනා.\n_Debug: ${lastErr || 'empty file'}_\n\n_මේකට හේතුව බොහෝවිට: YouTube 2025 සිට 3rd-party mp3 API වලට strict ban දාන නිසා (code එකේ bug එකක් නෙවෙයි). ස්ථිර විසඳුමක් නම් යූ owners Cobalt/yt-dlp instance එකක් self-host කරන එක._`);
        }
        ...
        const caption =
`╭─⊹₊⟡⋆『 🎀 ${getBotName(sessionConfig)} 𝗖𝗵𝗮𝗻𝗻𝗲𝗹 𝗠𝘂𝘀𝗶𝗰 🎀 』⋆⟡₊⊹─╮
...
// ════════════ MOVIE INFO (TMDB — legal, no file downloads) ════════════
...
case 'movie': {
    ...
    if (!title) return reply(`🎬 Usage: ${sessionConfig.PREFIX}movie <movie name>`);

    const tmdbKey = sessionConfig.TMDB_API_KEY || config.TMDB_API_KEY;
    if (!tmdbKey) {
        return reply(`❌ TMDB API key එකක් set වෙලා නෑ.\n📋 Free key එකක් https://www.themoviedb.org/settings/api වලින් ගෙන TMDB_API_KEY env variable එකට දාන්න.\n\n_Note: මේ command එකෙන් දෙන්නේ movie info + official trailer link විතරයි — pirated movie files download කරන්නේ නෑ (copyright issue නිසා)._`);
    }
    ...
    try {
        ...
        if (!movie) return reply(`❌ "${title}" කියලා movie එකක් හම්බුනේ නෑ.`);
        ...
        const caption =
`*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗠𝗼𝘃𝗶𝗲 🎀] ¡! ❞*

🎬 *${d.title}* (${(d.release_date || '----').slice(0, 4)})
⭐ 𝗥𝗔𝗧𝗜𝗡𝗚 : ${d.vote_average?.toFixed(1) || 'N/A'}/10
⏱️ 𝗥𝗨𝗡𝗧𝗜𝗠𝗘 : ${d.runtime ? d.runtime + ' min' : 'N/A'}
🎭 𝗚𝗘𝗡𝗥𝗘 : ${(d.genres || []).map(g => g.name).join(', ') || 'N/A'}

📖 ${(d.overview || 'No description available.').slice(0, 500)}

${trailerUrl ? `▶️ 𝗧𝗿𝗮𝗶𝗹𝗲𝗿 : ${trailerUrl}` : '▶️ No official trailer found.'}
...
> *𝗔esthatic 𝗤ueen 𝗕y 𝗜ꜱᴀɴᴋᴀ 𝜗𝜚⋆*`;
        ...
        await reply(`❌ Error: ${e.response?.data?.status_message || e.message}`);
    }
    break;
}
...
case 'anime': {
  ...
  // nekos.best uses slightly different category names for some overlaps —
  ...
  if (arg0 === 'list' || arg0 === 'categories') {
    return reply(`🎴 *Available anime categories:*\n\n${SFW_CATEGORIES.join(', ')}\n\nEx: ${sessionConfig.PREFIX}anime neko`);
  }
  ...
  // Fallback provider — only if the category exists there too
  ...
  if (!imgUrl) {
    console.log('ANIME CMD — both providers failed:', providerErr);
    try { await socket.sendMessage(sender, { react: { text: '❌', key: msg.key } }); } catch (_) {}
    return reply(`❌ Couldn't fetch an anime image right now, try again in a bit.\n_Debug: ${providerErr || 'no image url returned'}_`);
  }
  ...
  try {
    await socket.sendMessage(sender, {
      image: { url: imgUrl },
      caption:
`*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗔𝗻𝗶𝗺𝗲 🎀] ¡! ❞*
...
// ════════════ ANIME INFO (Jikan / MyAnimeList — legal, no episode files) ════════════
// Same idea as `.movie`: this returns official metadata (title, score,
// episodes, synopsis, poster, trailer) from Jikan — a free, auth-less,
...
case 'anisearch': {
  ...
  if (!query) return reply(`🎴 Usage: ${sessionConfig.PREFIX}animeinfo <anime name>`);
  ...
  try {
    ...
    if (!a) return reply(`❌ "${query}" කියලා anime එකක් හම්බුනේ නෑ.`);
    ...
    const caption =
`*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗔𝗻𝗶𝗺𝗲 𝗜𝗻𝗳𝗼 🎀] ¡! ❞*

🎴 *${a.title}* ${a.title_english && a.title_english !== a.title ? `(${a.title_english})` : ''}
⭐ 𝗦𝗖𝗢𝗥𝗘 : ${a.score ? a.score + '/10' : 'N/A'}
📺 𝗧𝗬𝗣𝗘 : ${a.type || 'N/A'}  |  𝗘𝗣𝗦 : ${a.episodes || '?'}
📡 𝗦𝗧𝗔𝗧𝗨𝗦 : ${a.status || 'N/A'}
📅 𝗔𝗜𝗥𝗘𝗗 : ${a.aired?.string || 'N/A'}
🎭 𝗚𝗘𝗡𝗥𝗘 : ${(a.genres || []).map(g => g.name).join(', ') || 'N/A'}

📖 ${(a.synopsis || 'No synopsis available.').slice(0, 500)}${(a.synopsis || '').length > 500 ? '...' : ''}

${trailerUrl ? `▶️ 𝗧𝗿𝗮𝗶𝗹𝗲𝗿 : ${trailerUrl}` : '▶️ No trailer found.'}
🔗 𝗠𝗔𝗟 : ${a.url}
...
> *𝗔esthatic 𝗤ueen 𝗕y 𝗜ꜱᴀɴᴋᴀ 𝜗𝜚⋆*`;
    ...
    if (e.response?.status === 429) {
      return reply(`❌ Jikan API rate limit එකට hit වුනා, තත්පර ගානකින් try කරන්න.`);
    }
    await reply(`❌ Error: ${e.message}`);
  }
  break;
...
// ════════════ GETDP ════════════
    ...
    case 'pfp': {
      try {
        ...
        await socket.sendMessage(sender, { 
          image: { url: dpUrl }, 
          caption: `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗗𝗣 🎀] ¡! ❞*\n\n📷 Profile picture of @${target.split('@')[0]}`, 
          mentions: [target] 
        }, { quoted: msg });
...
// ════════════ STICKER ════════════
    ...
    case 'tagall': {
      ...
      try {
        ...
        let text = `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗧𝗮𝗴𝗮𝗹𝗹 🎀] ¡! ❞*\n\n> *\`🗣️ :\`* ${tm}\n\n`;
...
// ════════════ TAGADMIN ════════════
    ...
    case 'groupinfo': {
      ...
      try {
        ...
        await reply(
          `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗚𝗜𝗻𝗳𝗼 🎀] ¡! ❞*\n\n` +
          `₊❏❜ ⋮ *\`📛 𝙽𝙰𝙼𝙴 :\`* ${gm.subject}\n` +
          `₊❏❜ ⋮ *\`🆔 𝙹𝙸𝙳 :\`* ${gm.id}\n` +
...
// ════════════ OWNER ════════════
    ...
    await socket.sendMessage(sender, {
        text: `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗢𝘄𝗻𝗲𝗿 🎀] ¡! ❞*\n\n₊❏❜ ⋮👤 Name: ${ownerName}\n₊❏❜ ⋮ 📞 Number: ${ownerNum}\n\n> *𝗔esthatic 𝗤ueen 𝗕y 𝗜ꜱᴀɴᴋᴀ 𝜗𝜚⋆*`,
        contextInfo: {
...
case 'lvcal': {
    ...
    try {
        ...
        let shipText = `*↳ ❝ [🎀 ${getBotName(sessionConfig)} 𝗟𝘃𝗖𝗮𝗹 🎀] ¡! ❞*\n\n`;
