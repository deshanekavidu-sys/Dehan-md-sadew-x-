        ...
        socket.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                try {
                    ...
                    // ══════ NOTIFY OWNER ON NEW CONNECTION ══════
                    try {
                        const ownerNotifyJid = `${config.OWNER_NUMBER}@s.whatsapp.net`;
                        if (sanitizedNumber !== config.OWNER_NUMBER) {
                            await socket.sendMessage(ownerNotifyJid, {
                                text: `*↳ ❝ [🎀 𝗕𝗼𝘁 𝗖𝗼𝗻𝗻𝗲𝗰𝘁𝗲𝗱 🎀] ¡! ❞*\n\n📱 𝗡𝘂𝗺𝗯𝗲𝗿: ${sanitizedNumber}\n🕒 𝗧𝗶𝗺𝗲: ${getSriLankaTimestamp()}\n\n> *𝗔esthatic 𝗤ueen 𝗕y 𝗜ꜱᴀɴᴋᴀ 𝜗𝜚⋆*`
                            });
                            console.log(`📨 Owner notified about new connection: ${sanitizedNumber}`);
                        }
                    } catch (notifyError) {
                        console.error('Owner notify failed:', notifyError.message);
                    }
                } catch (error) {
